...
// import knowledgeKnookLogo from "../../assets/Images/Knowledge-Knook(logo).jpeg";
...
import { AiOutlineShoppingCart } from "react-icons/ai";
import { MdKeyboardArrowDown, MdOutlineLightMode, MdOutlineDarkMode } from "react-icons/md";
...
const Navbar = () => {
  ...
  const [showNavbar, setShowNavbar] = React.useState("top");
  ...
  return (
    <nav
      className={`z-[10] flex h-14 w-full items-center justify-center border-b-[1px] border-b-richblack-700 text-white translate-y-0 transition-all ${showNavbar} ${theme === 'dark' ? 'bg-black' : 'bg-white text-black'}`}
    >
      <div className="flex w-11/12 max-w-maxContent items-center justify-between ">
        <Link to="/">
          <img
        ...
        <div className="flex gap-x-4 items-center">
          ...
          <button
            onClick={toggleTheme}
            className="ml-4 p-2 rounded-full bg-gray-300 hover:bg-gray-400"
          >
            {theme === 'dark' ? (
